...
vec4 seeThrough(float yc, vec2 p, mat3 rotation, mat3 rrotation) {
    ...
    if (yc <= 0.0 && (point.x < 0.0 || point.y < 0.0 || point.x > 1.0 || point.y > 1.0)) {
        return texture(dest, qt_TexCoord0);
    }

    if (yc > 0.0) return texture(source, p);

    vec4 color = texture(source, point.xy);
...
vec4 behindSurface(float yc, vec3 point, mat3 rrotation) {
    ...
    return vec4(texture(dest, qt_TexCoord0).rgb - shado, 1.0);
}
...
void main(void) {
    ...
    if (yc < 0.0) {
        ...
        otherColor = texture(source, qt_TexCoord0);
    }
